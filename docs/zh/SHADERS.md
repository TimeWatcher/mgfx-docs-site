...
`lux-mgfx/lux/mgfx/shaderpack/src/cl_module.lux`。`dist/lua` 下的 Plain GLua loader
构建会通过 `precompiled/` 项目消费同一个生成后的 shaderpack。

在 `lux-mgfx/lux/mgfx/shadersrc` 下常用的维护命令：
