...
建议先阅读 [在 Lux 中使用 MGFX](./USAGE)。公开渲染行为见 [API 总览](./API) 和
[详细 API 参考](./api-reference/)。

## 构建期形状

MGFX 源码位于 Lux package 树：
...
`packages/lux/mgfx/shadersrc` 保存 HLSL、已提交的 `.vcs` 产物和 shaderpack 生成器。
它是构建输入，不是 Lux module。二进制 shader compiler 位于 package 树之外的
`packages/tools/mgfx`。

## 运行时形状

`@lux/mgfx` 是 client-only package。根模块导入各功能模块，并把它们安装到一个 owner
table 上：
...
新 Lux 代码可以直接 import 并调用 module export。面向 GLua 的代码需要时可以使用
安装后的 `MGFX.*` facade。
...
| 表面 | 使用方 | 命名 |
| --- | --- | --- |
| Lux module API | Lux 源码 | `mgfx.paint.roundedBoxEx`、`mgfx.style.linearGradient` |
| installed facade | 旧 GLua panel、第三方代码、demo | `MGFX.RoundedBoxEx`、`MGFX.LinearGradient` |

lower-case module API 是主要实现表面。PascalCase facade 由各模块的 `install(owner)`
函数安装，用于贴合 GMod 习惯和迁移旧代码，并不代表 MGFX 内部依赖全局状态。

## 模块边界

`@lux/mgfx/src` 是 composition root。它连接功能模块、安装 public facade，并导出
submodule 供 Lux 直接 import。它应该保持编排层角色。

`@lux/mgfx/style` 负责 public style normalization：
...
`@lux/mgfx/paint` 是高层绘制层，在 roundrect、primitives、widgets、images 和 styles
之上提供常用 draw call。
